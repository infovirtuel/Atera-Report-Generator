...
global_config_file = 'globalconfig.ini'
...
login1 = configparser.ConfigParser()
globalconfig = configparser.ConfigParser()
globalconfig.read('globalconfig.ini')
login1.read('login.ini')

default_password = "ilovearg2023!"
default_username = "admin"
default_hashed_password = hashlib.sha3_512(default_password.encode()).hexdigest()
# Save the new username and password to the login.ini file
if not login1.has_section('admin'):
    login1.add_section('admin')
    login1.set('admin', default_username, default_hashed_password)
    with open('login.ini', 'w') as login_file:
        login1.write(login_file)
...
def create_global_config():
    ...
    with open(f'globalconfig.ini', 'w') as global_config_file:
...
def main_page() -> None:
    ...
    config_file = f'user_configs/config_{userlogin}.ini'
    ...
    config.read(f'user_configs/config_{userlogin}.ini')
    ...
    def make_atera_request(endpoint, method="GET", params=None):
        #config.read(f'user_configs/config_{userlogin}_apikey.enc')
        with open(f'user_configs/config_{userlogin}_apikey.enc', "rb") as f:
            encrypted_apikey = f.read(256)
        ...
        decrypted_api_key = cipher.decrypt(encrypted_apikey).decode()
    ...
    def create_config():
        ...
        with open(f'user_configs/config_{userlogin}.ini', 'w') as configfile:
    ...
    def fetch_device_information(search_options, search_values, teams_output,
        ...
        current_month = datetime.datetime.now().month
        current_day = datetime.datetime.now().day
        cache_directory = f"arg_cache/{userlogin}/atera/{output_mode}/{current_year}/{current_month}/{current_day}"
        ...
        try:
            ...
            while True:
                ...
                if cachemode == "True" and cache_mode_config == "True" or forcecachemode =="True":
    ...
    def output_results(found_devices, cli_mode,
        ...
        script_path = os.path.dirname(os.path.abspath(__file__))
        report_path = os.path.join(script_path, "reports")
        user_report_path_download = os.path.join(report_path, userlogin)
        pdf_path_download = os.path.join(user_report_path_download, 'pdf')
        csv_path_download = os.path.join(user_report_path_download, 'csv')
        excel_path_download = os.path.join(user_report_path_download, 'xlsx')
        ...
        if pdf_output:
            ...
            ui.download(f'http://{ipadd}:8080/reports/{userlogin}/pdf/{pdf_download}')
        ...
        if pdf_output:
            ...
            ui.timer(20.0, lambda: app.remove_route(f'/reports/{userlogin}/pdf'), once=True)
    ...
    app.storage.user.update({'geolocation': saved_geolocation_option})

    saved_onlineonly_option = config['GENERAL']['onlineonly']
    app.storage.user.update({'onlineonly': saved_onlineonly_option})

    saved_webhook_option = config['GENERAL']['teams_webhook']
    app.storage.user.update({'teams_webhook': saved_webhook_option})

    saved_eol_option = config['GENERAL']['eol']
    app.storage.user.update({'eol': saved_eol_option})

    saved_excel_option = config['GENERAL']['excel_output']
    app.storage.user.update({'excel': saved_excel_option})

    saved_cache_mode = config['GENERAL']['cachemode']
    app.storage.user.update({'cachemode': saved_cache_mode})

    saved_geoprovider_option = config['GENERAL']['geolocation_provider']
    app.storage.user.update({'geoprovider': saved_geoprovider_option})

    saved_email_recipient = config['EMAIL']['recipient_email']
    app.storage.user.update({'email_recipient': saved_email_recipient})

    saved_email_sender = config['EMAIL']['sender_email']
    app.storage.user.update({'email_sender': saved_email_sender})

    saved_email_subject = config['EMAIL']['subject']
    app.storage.user.update({'email_subject': saved_email_subject})

    saved_email_body = config['EMAIL']['body']
    app.storage.user.update({'email_body': saved_email_body})

    saved_smtp_server = config['SMTP']['smtp_server']
    app.storage.user.update({'smtp_server': saved_smtp_server})

    saved_smtp_port = config['SMTP']['smtp_port']
    app.storage.user.update({'smtp_port': saved_smtp_port})

    saved_smtp_username = config['SMTP']['smtp_username']
    app.storage.user.update({'smtp_username': saved_smtp_username})

    saved_smtp_password = config['SMTP']['smtp_password']
    app.storage.user.update({'smtp_password': saved_smtp_password})

    saved_ssl_option = config['SMTP']['ssl']
    app.storage.user.update({'ssl': saved_ssl_option})

    saved_starttls_option = config['SMTP']['starttls']
    app.storage.user.update({'starttls': saved_starttls_option})
    ...
    def delete_cache_folder():
        cache_directory = f"arg_cache/{userlogin}"
    ...
    def save_config(event=None):
        ...
        def save_general_config():
            ...
            if save_api_key != "":
                encryption_key_file = f'user_configs/config_{userlogin}_apikey.enc'
            ...
            with open(f'user_configs/config_{userlogin}.ini', 'w') as configfile:
                config.write(configfile)
            if save_api_key != "":
                with open(f'user_configs/config_{userlogin}_apikey.enc', 'wb') as f:
    ...
    with ui.row().classes('items-center self-center'):
        ui.label(f'Account: {app.storage.user["username"]}').classes('text-2xl')
        ui.button(on_click=logout_session, icon='logout').props('outline round color=pink-5')
        ui.button(on_click=change_password, icon='vpn_key').props('outline round color=pink-5')
        if app.storage.user.get('admin'):
            ui.button(on_click=lambda: (ui.open('/adminconsole')), icon='admin_panel_settings').props('outline round color=pink-5')
            ...
            with splitter.before:
                ...
                if teams_output_config == "True":
                    webhook = ui.input(label='Webhook URL', placeholder='https://......',
                                        validation={'Input too long': lambda value: len(value) < 500}).bind_value(app.storage.user, 'teams_webhook').classes('q-mb-md')
                geoprovider = ui.input(label='Geolocation Provider', placeholder='https://....',
                                        validation={'Input too long': lambda value: len(value) < 60}).bind_value(app.storage.user, 'geoprovider').classes('q-mb-md')

                ui.label('Advanced Report Options')
                with ui.row():
                    oseol = ui.checkbox('OS EOL')
                    geolocation = ui.checkbox('Geolocation').bind_value(app.storage.user, 'geolocation')
                with ui.row():
                    online_only = ui.checkbox('Online Devices').bind_value(app.storage.user, 'onlineonly')
                    xlsx_export = ui.checkbox('Excel File').bind_value(app.storage.user, 'excel')
                if cache_mode_config == "True" and forcecachemode == "False":
                    with ui.row():
                        cache_mode = ui.checkbox('Cache Mode').bind_value(app.storage.user, 'cachemode')

            with splitter.after:
                if email_output_config == "True":
                    ui.label('Email Options')
                    emailrecipient = ui.input(label='Email Recipient', placeholder='recipient@something.com',
                                  validation={'Input too long': lambda value: len(value) < 100}).bind_value(app.storage.user, 'email_recipient').classes('q-mb-md')
                    emailsender = ui.input(label='Email Sender', placeholder='sender@something.com',
                                  validation={'Input too long': lambda value: len(value) < 100}).bind_value(app.storage.user, 'email_sender').classes('q-mb-md')
                    emailsubject = ui.input(label='Email Subject', placeholder='Atera Report Results',
                                  validation={'Input too long': lambda value: len(value) < 100}).bind_value(app.storage.user, 'email_subject').classes('q-mb-md')

                    emailbody = ui.textarea(label='Email Body', placeholder='Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt',validation={'Input too long': lambda value: len(value) < 100}).bind_value(app.storage.user, 'email_body').classes('q-mb-md')
        with ui.splitter(horizontal=True) as splitter:
            with splitter.before:
                if email_output_config == "True":
                    smtpserver = ui.input(label='SMTP Server', placeholder='smtp.office365.com',validation={'Input too long': lambda value: len(value) < 100}).bind_value(app.storage.user, 'smtp_server').classes('q-mb-md')
                    smtpport = ui.input(label='SMTP Port', placeholder='587',validation={'Input too long': lambda value: len(value) < 100}).bind_value(app.storage.user, 'smtp_port').classes('q-mb-md')
                    with ui.row():
                        ssl = ui.checkbox('SSL').bind_value(app.storage.user, 'ssl')
                        starttls = ui.checkbox('TLS').bind_value(app.storage.user, 'starttls')

                    smtpusername = ui.input(label='SMTP Username', placeholder='sender@something.com',validation={'Input too long': lambda value: len(value) < 100}).bind_value(app.storage.user, 'smtp_username').classes('q-mb-md')
                    smtppassword = ui.input(label='SMTP Password', placeholder='Enter your Password Here', password=True,validation={'Input too long': lambda value: len(value) < 100}).bind_value(app.storage.user, 'smtp_password').classes('q-mb-md')
    ...
    ui.label('ARG Web Edition 1.5.5').classes('self-center')
...
def login() -> None:
    ...
    def try_login() -> None:
        ...
        hashed_password = hashlib.sha3_512(password_value.encode()).hexdigest()
        hashed_password_encrypt = hashlib.sha3_384(password_value.encode()).hexdigest()
        if login1.has_option('users', username_value):
            stored_password = login1.get('users', username_value)
            if stored_password == hashed_password:
                app.storage.user.update({'username': username_value, 'authenticated': True,'encrypted_password': hashed_password_encrypt})
                ui.open('/')
                return
        if login1.has_option('admin', username_value):
            stored_password = login1.get('admin', username_value)
            if stored_password == hashed_password:
                app.storage.user.update({'username': username_value, 'authenticated': True, 'admin': True,'encrypted_password': hashed_password_encrypt})
                ui.open('/')
                return
    ...
    with ui.card().classes('absolute-center').style('background-color: #FF176B'):
        ...
        ui.button('Admin Console', on_click=admin_panel, icon='admin_panel_settings').props('color=blue-5').classes('self-center')
        ui.label('Web Edition 1.5.5').classes('self-center').tailwind.text_color('white')
...
def change_password_page() -> None:
    ...
    def change_password() -> None:
        ...
        hashed_password = hashlib.sha3_512(new_password.encode()).hexdigest()
        os.remove(f'user_configs/config_{userlogin}_apikey.enc')

        if len(new_password) < 8:
            ui.notify('Password must be at least 8 characters long', color='negative')
            return

        if login1.has_option('admin', userlogin):
            login1.set('admin', userlogin, hashed_password)
        if login1.has_option('users', userlogin):
            login1.set('users', userlogin, hashed_password)
        with open('login.ini', 'w') as login_file:
            login1.write(login_file)
    ...
    with ui.card().classes('absolute-center').style('background-color: #FF176B'):
        ...
        ui.label('Password Change').classes('self-center text-2xl').tailwind.text_color('white')
        ...
        ui.button('Back', on_click=main_page, icon='reply').props('color=red-5').classes('self-center')
        ui.label('Web Edition 1.5.5').classes('self-center').tailwind.text_color('white')

@ui.page('/create-login')
def create_login_page() -> None:
    ...
    def create_login() -> None:
        ...
        if login1.has_option('users', new_username):
        ...
        hashed_password = hashlib.sha3_512(new_password.encode()).hexdigest()
        # Save the new username and password to the login.ini file
        if not login1.has_section('users'):
            login1.add_section('users')
        login1.set('users', new_username, hashed_password)
        with open('login.ini', 'w') as login_file:
            login1.write(login_file)
    ...
    with ui.card().classes('absolute-center').style('background-color: #FF176B'):
        ...
        if registration == "False":
            ui.label('User Registration is disabled').classes('self-center').tailwind.text_color('white')
...
def login() -> None:
    ...
    with ui.card().classes('absolute-center').style('background-color: #FF176B'):
        ui.image(login_img).classes('max-w-md self-center')
        ui.label('Admin Console Login').classes('self-center text-2xl').tailwind.text_color('white')
        username = ui.input('Username').props('color=grey-1').on('keydown.enter', try_login).classes('self-center w-full')
        password = ui.input('Password', password=True, password_toggle_button=True).props('color=grey-1').on('keydown.enter', try_login).classes('self-center w-full')
        ui.button('Login', on_click=try_login, icon='login').props('color=green-5').classes('self-center')
        ui.button('Back', on_click=login_page, icon='reply').props('color=red-5').classes('self-center')
...
def adminconsole() -> None:
    ...
    def delete_all_configurations():
        config_directory = f"user_configs"

        # Check if cache directory exists
        if os.path.exists(config_directory):
            # Remove the cache directory and all its contents
            shutil.rmtree(config_directory)
            ui.notify('Successfully flushed all configurations', type='positive')

    def delete_user_config(username):
        os.remove(f'user_configs/config_{username}.ini')
        os.remove(f'user_configs/config_{userlogin}_apikey.enc')


    def delete_user_cache(username):
        user_cache_directory = f"arg_cache/{username}"
        if os.path.exists(user_cache_directory):
            # Remove the cache directory and all its contents
            shutil.rmtree(user_cache_directory)
    def delete_user_report_folder(username):
        user_report_directory = f"reports/{username}"
    ...
    def create_login() -> None:
        ...
        if login1.has_option('admin', new_username):
            ui.notify('Username already exists', color='negative')
            return
        if login1.has_option('users', new_username):
            ui.notify('Username already exists', color='negative')
        ...
        if not login1.has_section('users'):
            login1.add_section('users')
        if new_type == "admin":
            login1.set('admin', new_username, hashed_password)
        if new_type == "user":
            login1.set('users', new_username, hashed_password)
        with open('login.ini', 'w') as login_file:
            login1.write(login_file)
    ...
    with ui.card().classes('absolute-center').style('background-color: #FF176B'):
        ...
        ui.label(f'Account: {app.storage.user["username"]}').classes('text-xl self-center').tailwind.text_color('white')
        ...
        ui.label('Web Edition 1.5.5').classes('self-center').tailwind.text_color('white')
    with ui.right_drawer(fixed=False).style('background-color: #FF176B').props('bordered overlay=True').classes('') as right_drawer:
        ...
        users_data = []
        for username in login1.options('users'):
            users_data.append({'name': username, 'type': 'users'})
        for username in login1.options('admin'):
            users_data.append({'name': username, 'type': 'admin'})
        ...
        async def delete_selected_rows_users():
            ...
            if selected_rows:
                for row in selected_rows:
                    username = row['name']
                    login1.remove_option('users', username)
                    login1.remove_option('admin', username)
                    delete_user_config(username)
                    delete_user_cache(username)
                    delete_user_report_folder(username)
                with open('login.ini', 'w') as login_filez:
                    login1.write(login_filez)
        ...
        async def delete_selected_rows_user_config():
            ...
            if selected_rows:
                for row in selected_rows:
                    username = row['name']
                    delete_user_config(username)
                ui.notify('Users configuration deleted successfully. Please refresh the page.', color='positive')
            else:
                ui.notify('No user selected.', color='warning')
        ...
        async def delete_selected_rows_user_cache():
            ...
            if selected_rows:
                for row in selected_rows:
                    username = row['name']
                    delete_user_cache(username)
                ui.notify('Users cache deleted successfully. Please refresh the page.', color='positive')
            else:
                ui.notify('No user selected.', color='warning')
        ...
        ui.button('Flush User config', on_click=delete_selected_rows_user_config, icon='warning').classes('q-mt-md self-center').props('color=orange-5')
    ...
    with ui.left_drawer(fixed=False).style('background-color: #FF176B').props('bordered overlay=True').classes('') as left_drawer:

        ui.label('Global Configuration').classes('self-center text-2xl').tailwind.text_color('white')
        ...
        app.storage.user.update({'registration': registration_state})
        app.storage.user.update({'cachemode': cache_state})
        app.storage.user.update({'teams': teams_state})
        app.storage.user.update({'email': email_state})
        app.storage.user.update({'forcecache': force_cache_state})
        ui.label('Option visibility').classes('self-center text-lg').tailwind.text_color('white')
        registration_checkbox = ui.checkbox('Enable User Registration').bind_value(app.storage.user, 'registration').classes('self-center')
        teams_checkbox = ui.checkbox('Enable Teams Output').bind_value(app.storage.user,'teams').classes('self-center')
        email_checkbox = ui.checkbox('Enable Email Output').bind_value(app.storage.user, 'email').classes('self-center')
        ui.label('Parameters').classes('self-center text-lg').tailwind.text_color('white')
        cachemode_checkbox = ui.checkbox('Enable Cache Mode').bind_value(app.storage.user,'cachemode').classes('self-center')
        forcecachemode_checkbox = ui.checkbox('Force Cache Mode').bind_value(app.storage.user, 'forcecache').classes('self-center')
        ...
        profile_flush = ui.button('Delete users config', on_click=delete_all_configurations, icon='warning').classes('q-mt-md self-center').props('color=red-5')
...
ui.run(title='Atera Report Generator',host='0.0.0.0', favicon=icon, reload=False, storage_secret='#6SdZ4FiZe*ago@yj#^WK8bsJxG@&j%Y534rLUxw!B%@tHzCV')
import warnings
